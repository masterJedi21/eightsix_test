/* ════════════════════════════════════════════════════════════════
   ...
   Scroll reveals, counters, tilt/glow cards, parallax, particles,
   FAQ accordion, header state, progress bar, back-to-top.
   Everything degrades gracefully without JS and honors
...
(function () {
    ...
    var animOn = !reduced && 'IntersectionObserver' in window;
    ...
    /* Everything below is motion — bail out for reduced motion / old browsers */
    if (!animOn) return;
    docEl.classList.add('e86-anim');

    // Bars carry inline widths as a no-JS fallback; zero them out so the
    // fill can animate in when they scroll into view.
    document.querySelectorAll('.e86-bar[data-bar]').forEach(function (el) {
        el.style.width = '0%';
    });

    /* ───────────────────────────── Auto-tag scroll reveals ──
       Pages get scroll animation without hand-tagging every node.
       Explicit data-reveal attributes always win. */
    ...
    function autoTag() {
        ...
        // Section headings + the eyebrow/paragraph around them
        ...
        // Grid children, staggered — but recurse past wrapper columns that
        // contain their own grids so inner cards animate individually
    ...
    // Line-draw elements observed separately (they may not be data-reveal)
    ...
    // counters not nested in a reveal element still animate on view
